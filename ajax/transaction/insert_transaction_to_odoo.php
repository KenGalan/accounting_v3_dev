...
'' reference,
maad.item_label,
TO_CHAR(maa.accounting_date, 'MM/DD/YYYY') last_date_of_month,
aj.name journal,
am.journal_id,
maad.account_code,
...
aa.root_id,
false is_wip,
coalesce(maad.cogs_Account_id, NULL::INTEGER) cogs_account_id
FROM
m_acc_cust_dist maa 
join m_acc_accrual macc on macc.id =maa.accrual_id
join m_acc_distribution_transaction maad on maad.transaction_id = maa.id and transaction_type = 'custom_distribution'
left join account_account aa on aa.id = maad.account_id
	left join account_move am on am.id = maa.move_id
	left join account_journal aj on aj.id = am.journal_id
	where maa.id in ($cd_ids)
ORDER BY maa.id,COALESCE(maad.credit,0) desc,COALESCE(maad.debit,0) DESC, maad.account_code	
...
aa.id account_id,
atc.analytic_account_id,
atc.analytic_account,
sum(COALESCE(atc.debit,0)) debit,
sum(COALESCE(atc.credit,0)) credit,
aa.root_id,
true is_wip,
atc.account_id cogs_account_id
from m_acc_trx_cogs atc
join m_acc_cust_dist maa on maa.id = atc.transaction_id 
join m_acc_accrual macc on macc.id =maa.accrual_id
join account_account aa on aa.id = atc.account_id
		left join account_move am on am.id = maa.move_id
	left join account_journal aj on aj.id = am.journal_id
-- join M_ACC_MONTH ADR on ADR.id = maa.MONTH_ID
-- where adr.id =var(month_id)
	where maa.id  in ($cd_ids)
	group by
	maa.id,
TO_CHAR(maa.accounting_date, 'MM/DD/YYYY') ,
aj.name,
aj.id,
aa.code ,
aa.id ,
        ...
        foreach ($result as $row) {
            ...
            if ($account_move_checker != $account_move_checker_new) {
                ...
                if ($is_wip == 't') {
                    ...
                    // if ($wip_account_id) {
                    $qGetMosLine =  getMOS($month_id,  $transaction_id, '', '', false, $transaction_type, '');

                    // getMOS($month_id, $accrual_id, $sbu, $is_wip, $from_prev, $transaction_type, $cogs_account_id)
                    ...
                    foreach ($resMosLine as $mosLine) {
                        $mo_line_entries = [
                            ...
                            'ACTUAL_ALLOCATION' =>  $mosLine['allocation'],
                            'COGS_ACCOUNT_ID' => $mosLine['cogs_account_id']
                        ];
                    ...
                    $qUpdateAccrual = "UPDATE m_acc_cust_dist SET distributed_account_move_id =$1 WHERE ID = $2";
                }

                $db_ken->query(
                    $qUpdateAccrual,
                    [$new_am_id, $transaction_id]
                );
            ...
            if ($new_am_id) { // IF IT HAS ACCOUNT_MOVE_ID
                ...
                if ($new_aml_id) { // if aml id has value
                    ...
                    if ($sbu_result != "" && $cogs_account_id) {


                        $qGetMos =  getMOS($month_id,  $accrual_id, $sbu_result, $is_wip, true, $transaction_type, $cogs_account_id);
...
function getMOS($month_id, $accrual_id, $sbu, $is_wip, $from_prev, $transaction_type, $cogs_account_id)
{

    $cogs_params = $cogs_account_id == '' ? 'WIP_ACCOUNT_ID' : $cogs_account_id;
    if ($is_wip == 't' &&  $from_prev == 1) {
        $addQuery = ", for_sum as (	
            ...
            union all
             ...
             join m_acc_accrual ma2 on ma2.id = adml.accrual_id AND  MA2.transaction_type = '$transaction_type'
    ...
    //     from 
    //     m_acc_month adr
    //     join M_ACC_ACCRUAL maa on maa.month_id = adr.ID AND  MAA.IS_ACCRUAL = '$transaction_type'
    //     join (
    ...
    //     JOIN M_ACC_ACCRUAL a_main on a_main.id = a.accrual_id AND A_MAIN.IS_ACCRUAL = '$transaction_type'
    ...
    // and cogs_account_id = $cogs_params
    // )
    // $addQuery
    // ";
...
from 
m_acc_month adr
join M_ACC_ACCRUAL maa on maa.month_id = adr.ID AND  MAA.transaction_type = '$transaction_type'
join (
    select  b.accrual_id, a.analytic_account, a.analytic_account_id, sum(a.distribution_percentage) distribution_percentage,a.sbu, sum(a.debit) debit,sum(a.credit) credit,
    a_main.from_date,
    a_main.to_date,
    A.cogs_ACCOUNT_ID,
    B.MO_DIST mo_pct_ref
    from m_acc_distribution_transaction a
    join m_acc_cust_dist b on b.id = a.transaction_id
    JOIN M_ACC_ACCRUAL a_main on a_main.id = b.accrual_id AND A_MAIN.transaction_type = '$transaction_type'
    WHERE b.ACCRUAL_ID = $accrual_id
    and a.cogs_ACCOUNT_ID IS NOT NULL
    group by  b.accrual_id, a.analytic_account, a.analytic_account_id,a.sbu,a_main.from_date,A.cogs_ACCOUNT_ID,B.MO_DIST,
    a_main.to_date order by analytic_account_id
) aad on aad.accrual_id = maa.id	
join account_analytic_account aaa on aaa.id =aad.analytic_account_id
join m_acc_depARTMENT_groups adg on adg.id = aaa.m_acc_group_id
...
NT.CATEGORY,
NT.ALLOCATION,
case when 'f' = 't' then null::numeric else nt.percentage end percentage,
NT.CUSTOMER_NAME,
NT.earned_hrs,
TRUNC(NT.ALLOCATION,5) TRUNC_ALLOCATION,
SUM(NT.ALLOCATION) OVER() TOTAL_ALLOCATION,
...
MO_RANKED
where lower(sbu) = case when '$sbu' = '' then lower(sbu) else lower('$sbu') end
and is_invoiced = case when 'f' = 't' then true else is_invoiced end
and COGS_ACCOUNT_ID = $cogs_params
)
$addQuery
";
