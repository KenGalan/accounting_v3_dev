        ...
        foreach ($result as $row) {
            ...
            if ($new_am_id) { // IF IT HAS ACCOUNT_MOVE_ID
                ...
                if ($new_aml_id) { // if aml id has value
                    ...
                    $qGetMos =  getMOS($month_id,  $accrual_id);
                    $resMos = $db_ken->fetchAll($qGetMos);
                    foreach ($resMos as $mos) {
                        //insert all mos to M_ACC_DIST_MO_LINES
                        $mo_line_entries = [
                            'DIST_MO_ID' =>    $mos['dist_mo_id'],
                            'ACCRUAL_ID' => $mos['accrual_id'],
                            'ACCRUAL_ALLOCATION' =>  $mos['allocation']
                        ];

                        $db_ken->insert('M_ACC_DIST_MO_LINES', $mo_line_entries);

                        if ($sbu_result != "" && $sbu_result == strtolower($mos['sbu'])) {

                            if ($is_wip == 't') {
                                if ($mos['is_invoiced'] == 't') {
...
function getMOS($month_id, $accrual_id)
{
...
group by  a.accrual_id, a.analytic_account, a.analytic_account_id,a.sbu,MACT.mo_pct_ref order by analytic_account_id) aad on aad.accrual_id = maa.id	
