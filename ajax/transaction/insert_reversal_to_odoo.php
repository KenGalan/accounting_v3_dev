            ...
            if ($account_move_checker != $account_move_checker_new) {
                ...
                if ($is_wip == 't') {
                    ...
                    foreach ($resMosLine as $mosLine) {
                        ...
                        $qUpdateMoLines = "UPDATE M_ACC_MO_WIP_LINE SET reversed_allocation =$1 , actual_allocation = case when $2 = 0 then accrual_allocation - $1 else accrual_allocation + $1 end WHERE ACCRUAL_ID = $3 AND MO_WIP_ID = $4";
                        $db_ken->query(
                            $qUpdateMoLines,
                            [$mosLine['allocation'], $mosLine['is_debit'], $mosLine['accrual_id'], $mosLine['mo_wip_id']]
                        );
                    }
                    // }
                }
...
join m_acc_mo_wip adm on adm.sbu =aad.sbu and adm.month_id = adr.id and adm.from_date = maa.from_date and adm.to_date = maa.to_date
