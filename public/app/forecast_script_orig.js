$(document).ready(function() {
  ...
  function getForecastDataTable(){
    ...
    $.ajax({
        ...
        url: "ajax/forecast/select/select_forecast.php", 
        success: function(response){
          ...
          $(header).each(function(){
            self = this;
            if(i < 7){
              headerString += "<th rowspan='2'>"+self+"</th>";
            }
            else if(i == (header.length - 1)){
              headerString += "<th rowspan='2'>"+self+"</th>";
            }
            else{
              headerString += "<th colspan='3'>"+self+"</th>";
              count++;
            }
          ...
          index = 7;
          for(j=0;j<count; j++){
            headerString += "<th>Forecast</th>";
            headerString += "<th>Plan</th>";
            //headerString += "<th>Capacity</th>";
            //headerString += "<th>Available</th>";
            headerString += "<th>Actual</th>";
            if(j != (count-1)){
              index = index + 4;
              indexString += ", "+index;
            }
          ...
          headerString += "</tr>";
          $("#forecastTableHead").html(headerString+subheaderString);
          body = response.body;
          if(body != null){
          bodyString= "";
          $(body).each(function(){
            ...
            $(self).each(function(){
              self1 = this;
              if(j == 9){
                bodyString+="<td><a>"+self1+"</a></td>";
              }
              else{
                bodyString+="<td>"+self1+"</td>";
                //bodyString+="<td>---</td>";
              }
              j++;
            ...
            if(is_mis == 1){
              bodyString+="<button class='btn btn-sm bg-purple forecastTblBtns' title='add/edit this forecast by batch' id='addMonthForecastBatchBtn'>"
              +"<i class='material-icons' style='font-size: 1.5rem;'>open_in_new</i>"
              +"</button>";
            }
            bodyString+="</td>";
            // +"<button id='forecastActionBtnGrp' type='button' class='btn btn-sm btn-info waves-effect dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'><i class='material-icons'>open_in_new</i></button>"
            // +"<ul class='dropdown-menu' aria-labelledby='forecastActionBtnGrp'>"
            // +"<li><a href='javascript:void(0);'>Add/Edit Individually</a></li>"
            // +"</ul>"
            // +"</td>";
            bodyString+="</tr>";
          });
          console.log(indexString);
          $("#forecastTableBody").html(bodyString);
          forecastDataTable();
